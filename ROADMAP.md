...
- Agent skill layer: six skills that turn `iecli` JSON into narrative answers, shipped to more than one agent so contributors can pick their own — [`skills/`](./skills/) is the canonical Codex / Open Agent Skills package set and [`.claude/skills/`](./.claude/skills/) is a generated Claude Code mirror, kept in step by `scripts/skill_parity.py` and enforced in CI. Inventory and per-script flags live in [docs/SKILLS.md](./docs/SKILLS.md).
