...
- **DLC archives (`dlc/*.zip`).** The loader mounts Enhanced Edition DLC overlays read-only: KEY-backed BIFs can resolve from a zip, game overrides outrank DLC overrides, later-sorted DLCs win among DLC overrides, and the largest matching DLC TLK is selected. `source_kind: "dlc"` and a `zip!interior/path` source identify packed resources. DlcMerger is not emulated or required.
