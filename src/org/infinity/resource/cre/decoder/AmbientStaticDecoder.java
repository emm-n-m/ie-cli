...
/**
 * Creature animation decoder for processing type B000 (ambient_static) animations. Available ranges: [b000,bfff]
 */
public class AmbientStaticDecoder extends SpriteDecoder {
  ...
  static {
    ...
    SUFFIX_MAP.put(Sequence.STAND, Couple.with("G1", 8));
    SUFFIX_MAP.put(Sequence.GET_HIT, Couple.with("G1", 16));
    SUFFIX_MAP.put(Sequence.DIE, Couple.with("G1", 24));
    SUFFIX_MAP.put(Sequence.SLEEP, SUFFIX_MAP.get(Sequence.DIE));
    SUFFIX_MAP.put(Sequence.GET_UP, Couple.with("!G1", 24));
    SUFFIX_MAP.put(Sequence.TWITCH, Couple.with("G1", 32));
  }
