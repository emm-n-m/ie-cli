...
public class ColorGrid extends JPanel implements MouseListener, MouseMotionListener {
  ...
  private static final int MAX_COLOR_COUNT = 65536;
  private static final int DEFAULT_COLOR_COUNT = 16;
  private static final int DEFAULT_COLORS_PER_ROW = 16;
  private static final int DEFAULT_GAP = 3;
  private static final int DEFAULT_SELECTION_MODE = SELECTION_SINGLE;
  private static final Dimension DEFAULT_COLOR_SIZE = new Dimension(16, 16);
  private static final Color DEFAULT_COLOR = Color.BLACK;
  private static final Frame DEFAULT_FRAME = Frame.SINGLE_LINE;
  private static final Color DEFAULT_FRAME_COLOR = Color.BLUE;
  private static final boolean DEFAULT_DRAG_DROP_ENABLED = false;

  // Defines the square of the minimum dragging distance before activating drag&drop mode
  private static final int DRAG_DROP_TRIGGER_DISTANCE_2 = 16;

  // Background pattern is visible for semi-transparent color entries
  private static final TexturePaint BACKGROUND_PATTERN = createBackgroundPattern();
  ...
  /**
   * Adds an ActionListener to the component. ActionListener events will be triggered whenever a new color entry has
   * been selected.
   */
  ...
  /**
   * Adds a MouseOverListener to the component. MouseOverListener events will be triggered whenever the mouse cursor is
   * over a new color entry.
   */
  ...
  /**
   * Adds a ChangeListener to the component. ChangeListener events will be triggered whenever the palette definition of
   * the grid has been modified by the user.
   */
  ...
  public void setSelectionMode(int mode) {
    if (mode == SELECTION_SINGLE || mode == SELECTION_MULTIPLE && selectionMode != mode) {
      ...
      if (selectionMode == SELECTION_SINGLE && !listSelection.isEmpty()) {
        setSelectedIndex(listSelection.get(listSelection.size() - 1));
      }
    }
  }
  ...
  /**
   * Specify the size for each individual color entry displayed by the component. Specifying {@code null} restores the
   * default size.
   */
  public void setColorEntrySize(Dimension size) {
    if (size != null) {
      ...
      colorSize = new Dimension(DEFAULT_COLOR_SIZE);
    }
    updateSize();
  }
  ...
  /**
   * Specify a new number of colors to be displayed by this component. New color entries are always filled with the
   * default color "black".
   *
   * @param newCount The new number of colors to be displayed.
   */
  public void setColorCount(int newCount) {
    if (newCount > 0 && newCount < MAX_COLOR_COUNT && newCount != getColorCount()) {
  ...
  /**
   * Returns the color value at the selected color entry. In multiple selection mode only the last selected color value
   * will be returned. Returns {@code null} if no color has been selected.
   */
  public Color getSelectedColor() {
    if (!listSelection.isEmpty()) {
      return listColors.get(listSelection.get(listSelection.size() - 1));
    } else {
      return null;
    }
  }
  ...
  /**
   * Returns a list of all selected color entries. Returns an empty array if no color has been selected.
   */
  public Color[] getSelectedColors() {
    Color[] retVal = new Color[listSelection.size()];
    for (int i = 0; i < listSelection.size(); i++) {
      retVal[i] = listColors.get(listSelection.get(i));
    }
    return retVal;
  }
  ...
  /**
   * Returns the currently selected color in single selection mode or the last selected value in multiple selections
   * mode.. Returns -1 if no color has been selected.
   */
  public int getSelectedIndex() {
    if (!listSelection.isEmpty()) {
      return listSelection.get(listSelection.size() - 1);
    } else {
      return -1;
    }
  }
  ...
  public int[] getSelectedIndices() {
    ...
    for (int i = 0; i < listSelection.size(); i++) {
      retVal[i] = listSelection.get(i);
    }
    return retVal;
  }

  /**
   * Selects the color entry at the specified index. Previously selected entries will be unselected automatically.
   */
  ...
  /**
   * Adds the specified index to the current selection in multiple selection mode. Behaves like
   * {@link #setSelectedIndex(int)} in single selection.
   */
  ...
  /**
   * Adds the specified indices to the current selection in multiple selection mode. Behaves like
   * {@link #setSelectedIndex(int)} in single selection, but selects only the last entry in the specified index array.
   *
  ...
  protected void fireActionListener() {
    ...
    for (ActionListener listActionListener : listActionListeners) {
      listActionListener.actionPerformed(event);
    }
  }
  ...
  protected void fireMouseOverListener(int index) {
    ...
    for (MouseOverListener listMouseOverListener : listMouseOverListeners) {
      listMouseOverListener.mouseOver(event);
    }
  }
  ...
  private static TexturePaint createBackgroundPattern() {
    ...
    int[] checker = { 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffc0c0c0, 0xffc0c0c0, 0xffc0c0c0, 0xffc0c0c0,
  ...
  private void init(int count, Collection<Color> colors, Dimension colorDim) {
    ...
    if (count < 1) {
      count = 1;
    } else if (count > MAX_COLOR_COUNT) {
      count = MAX_COLOR_COUNT;
    }
    listColors.clear();
    int idx = 0;
    ...
    if (g != null) {
      ...
      for (int i = 0; i < getColorCount(); i++) {
        ...
        if (Platform.IS_UNIX) {
          ...
          g2d.setPaint(BACKGROUND_PATTERN);
          g2d.fillRect(x, y, colorSize.width, colorSize.height);
          g2d.setPaint(null);
        }
      ...
      for (Integer element : listSelection) {
        int curIdx = element;
  ...
  private int getColorIndexAt(Point coord) {
    if (coord != null && (coord.x - gapX) >= 0 && (coord.y - gapY) >= 0 && (coord.x - gapX) < getWidth()
        && (coord.y - gapY) < getHeight()) {
  ...
  private void updateColorDrag(Point coords) {
    if (coords != null) {
      if (DragDropMode == DRAG_INITIALIZED) {
        ...
        if (x * x + y * y >= DRAG_DROP_TRIGGER_DISTANCE_2) {
  ...
  private void updateColorLocation(int newIndex) {
    if (DragDropMode == DRAG_ENABLED) {
      if (newIndex >= 0 && dragDropCur != newIndex) {
        ...
        for (int i = 0; i < listSelection.size(); i++) {
          int idx = listSelection.get(i);
          if (idx < dragDropCur && idx >= newIndex) {
            listSelection.set(i, idx + 1);
  ...
  public static class MouseOverEvent extends EventObject {
     ...
     * @param index  The color index where the mouse cursor is currently located. Specify a negative value to indicate
     *               an invalid color entry.
     */
