...
public class BamFilterColorReplace extends BamFilterBaseColor implements ActionListener {
  private static final String FILTER_NAME = "Replace colors (BAM v1 only)";
  private static final String FILTER_DESC = "This filter allows you to manipulate individual palette "
  ...
  private BufferedImage applyEffect(BufferedImage srcImage) {
    ...
    if (srcImage != null && paletteDialog.isModified() && srcImage.getType() == BufferedImage.TYPE_BYTE_INDEXED) {
      int width = srcImage.getWidth();
      int height = srcImage.getHeight();
      IndexColorModel cm = (IndexColorModel) srcImage.getColorModel();
      int[] newPalette = paletteDialog.getPalette();

      IndexColorModel cm2 = new IndexColorModel(8, 256, newPalette, 0, cm.hasAlpha(), cm.getTransparentPixel(),
  ...
  private static class PaletteDialog extends JDialog
    ...
    /**
     * Returns whether the palette contains any modified data. (Either because an external palette has been loaded or
     * color entries have been modified.)
     */
        ...
        if ("BM".equals(new String(signature, 0, 2))) {
          ...
          if ("BAM ".equals(sig) || "BAMC".equals(sig)) {
            if ("V1  ".equals(ver)) {
              ...
              throw new Exception(
                  String.format("BAM file \"%s\" does not contain palette data.", paletteFile.getFileName()));
            }
          } else {
